...
pub enum DPShifterOperand {
    Immediate{rotate: u8, immed: u8},
    ImmediateShift{immed: u8, shift_type: ShiftType, rm: usize},
    RegisterShift{rs: usize, shift_type: ShiftType, rm: usize},
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            &DPShifterOperand::ImmediateShift { immed, shift_type, rm } => 
                write!(f, "r{}, {} {}", rm, shift_type, immed),
            &DPShifterOperand::RegisterShift { rs, shift_type, rm } => 
            write!(f, "r{}, {} r{}", rm, shift_type, rs),
        }
    }
}
...
pub enum LoadStoreStatusOperation {
    StatusRegisterToRegister{rd: usize},
    RegisterToStatusRegister{mask: MSRMask, rm: usize},
...
pub enum BranchOperation {
    ...
    BranchLinkImmed { offset: i32 },
    BranchExchangeThumb { rm: usize },
    BranchExchangeLinkThumb { rm: usize },
    BranchExchangeJava { rm: usize },

}

pub enum ArmInstruction {
    DataProcessing{c: Condition, op: DataProcessingOpcode, s: bool, rn: u8, rd: u8, shifter_operand: DPShifterOperand},
    LoadStore{c: Condition, pre_indexed: bool, add_offset: bool, byte_access: bool, w: bool, load: bool, rn: u8, rd: u8, shifter_operand: LSShifterOperand},
    ...
    LoadStoreMultiple{c: Condition, p: bool, u: bool, s: bool, w: bool, l: bool, rn: u8, register_list: Vec<u8>},
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ArmInstruction::DataProcessing { c, op, s, rn, rd, shifter_operand } => {
                match op {
                    ...
                    DataProcessingOpcode::CMN =>
                        write!(f, "{}{} r{}, {}", op, c, rn, shifter_operand),
                    DataProcessingOpcode::MOV |
                    DataProcessingOpcode::MVN => 
                        write!(f, "{}{}{} r{}, {}", op, c, if s { "S" } else { "" }, rd, shifter_operand),
                    _ => 
                        write!(f, "{}{}{} r{}, r{}, {}", op, c, if s { "S" } else { "" }, rd, rn, shifter_operand)
                }
            },
            ArmInstruction::LoadStore { c, pre_indexed: p, add_offset: u, byte_access: b, w, load: l, rn, rd, shifter_operand } => {
                let mut t = "";
                let mut adr = String::new();
                
                if p {
                    if u {
                        adr = format!("[r{} {}]", rn, shifter_operand);
                    } else {
                        adr = format!("[r{} -{}]", rn, shifter_operand);
                    }
                } else {
            ...
            ArmInstruction::LoadStoreStatus { c, r, op } => {
                match op {
                    LoadStoreStatusOperation::StatusRegisterToRegister { rd } => 
                        write!(f, "MRS{} r{}, {}", c, rd, if r { "SPSR" } else { "CPSR" }),
                    LoadStoreStatusOperation::RegisterToStatusRegister { mask, rm } => 
                        write!(f, "MSR{} {}_{}, r{}", c, if r { "SPSR" } else { "CPSR" }, mask, rm),
            ...
            ArmInstruction::Branch {c, op} => match op {
                ...
                BranchOperation::BranchLinkImmed { offset } => write!(f, "BL{} {:#x}", c, offset),
                BranchOperation::BranchExchangeThumb { rm} => write!(f, "BX{} r{}", c, rm),
                BranchOperation::BranchExchangeLinkThumb { rm } => write!(f, "BLX{} r{}", c, rm),
                BranchOperation::BranchExchangeJava { rm } => write!(f, "BJ{} r{}", c, rm),
            }
            _ => write!(f, "UNKNOWN"),
        }
    }
...
impl GbaSystem {
    ...
    fn decode_data_processing_immediate<'b>(inst: (&'b[u8], usize)) -> IResult<(&'b[u8], usize), ArmInstruction> {
        let (i, (cond, _, opcode, s, rn, rd, rotate, immed)): ((&[u8], usize), (u8, u8, u8, bool, u8, u8, u8, u8)) = 
            tuple(
                (
                    take(4usize), 
                    tag(0b001, 3usize), 
    ...
    fn decode_load_store_immediate_offset<'b>(inst: (&'b[u8], usize)) -> IResult<(&'b[u8], usize), ArmInstruction> {
        ...
        Ok((i, ArmInstruction::LoadStore {
            c: Condition::try_from(cond).unwrap(), pre_indexed: p, add_offset: u, byte_access: b, w, load: l, rn, rd,
    ...
    fn decode_branch<'b>(inst: (&'b[u8], usize)) -> IResult<(&'b[u8], usize), ArmInstruction> {
        ...
        let mut offset_adr: i32 = (offset & 0x7FFFFF) as i32;
        offset_adr |= ((offset & 0x800000) << 8) as i32;
        offset_adr <<= 2;

        if l {
            Ok((i, ArmInstruction::Branch { 
                c: Condition::try_from(cond).unwrap(), 
                op: BranchOperation::BranchLinkImmed { offset: offset_adr }
            }))
        } else {
            Ok((i, ArmInstruction::Branch { 
                c: Condition::try_from(cond).unwrap(), 
    ...
    fn decode_move_status_register_to_register<'b>(inst: (&'b[u8], usize)) -> IResult<(&'b[u8], usize), ArmInstruction> {
        ...
        Ok((i, ArmInstruction::LoadStoreStatus { 
            c: Condition::try_from(cond).unwrap(), 
            r, 
            op: LoadStoreStatusOperation::StatusRegisterToRegister { rd: rd as usize } }))
    }
    ...
    fn decode_move_register_to_status_register<'b>(inst: (&'b[u8], usize)) -> IResult<(&'b[u8], usize), ArmInstruction> {
        ...
        Ok((i, ArmInstruction::LoadStoreStatus { 
            c: Condition::try_from(cond).unwrap(), 
            r, 
            op: LoadStoreStatusOperation::RegisterToStatusRegister { mask: MSRMask::from_bits(mask).unwrap(), rm: rm as usize } }))
    }
    ...
    fn decode_branch_exchange_java<'b>(inst: (&'b[u8], usize)) -> IResult<(&'b[u8], usize), ArmInstruction> {
        ...
        Ok((i, ArmInstruction::Branch { 
            c: Condition::try_from(cond).unwrap(), 
            op: BranchOperation::BranchExchangeJava { rm: rm as usize } 
        }))
    }
